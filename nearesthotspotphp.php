<?php
	...
	$addressSelect = $pdo->prepare("SELECT address, name FROM items WHERE latitude='".$_GET["latitude"]."' AND longitude='".$_GET["longitude"]."'");
	//fetch the reviews
	$addressSelect->execute();
	$addressData=$addressSelect->fetchAll(PDO::FETCH_ASSOC);
	$reviewsSelect = $pdo->prepare("SELECT email, review, rating, date_posted, title FROM reviews WHERE name='".$addressData['name']."'");
	$reviewsSelect->execute();
	$reviewsData=$reviewsSelect->fetchAll(PDO::FETCH_ASSOC);
	//Print the title
	echo "<div class='title'><h2>".$addressData["name"]."</h2>";
